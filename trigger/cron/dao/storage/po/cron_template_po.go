...
type CronTriggerTemplate struct {
	...
	CronExpr string `gorm:"column:cron_expr;type:varchar(255);not null;default:'';comment:'cron表达式'" copier:"must,nopanic"`

	// Topic the message topic
	Topic string `gorm:"column:topic;type:varchar(255);not null;default:null;comment:'消息Topic'" copier:"must,nopanic"`
	// Payload the message payload
	Payload []byte `gorm:"column:payload;type:blob;not null;default:null;comment:'消息内容'" copier:"must,nopanic"`

	// NextExecutionTime last time to schedule the message
	LastExecutionTime time.Time `gorm:"column:last_execution_time;type:datetime;not null;default:null;comment:'上次执行时间'" copier:"must,nopanic"`

	// ExceptedEndTime Excepted Trigger end time, if it is 0, it means that it will not end.
	ExceptedEndTime time.Time `gorm:"column:excepted_end_time;type:datetime;not null;default:null;comment:'预期结束时间'" copier:"must,nopanic"`
